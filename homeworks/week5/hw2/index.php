					<?
						...
						//cookie的內容：$_COOKIE["user_id"]
						//isset：如果有設置的話 //!empty如果不是空的話 //代表有登入
						$is_login = true;
						//有登入
        ...
        <div class="board">
			<div class="board__title">留言區
					...
					<?	
						...
						//查詢目前頁面需要的十筆主留言
						$sql = "SELECT jeanpang_comments.id, jeanpang_comments.content, jeanpang_comments.created_at, jeanpang_users.nickname FROM jeanpang_comments left join jeanpang_users on jeanpang_comments.user_id = jeanpang_users.id where parent_id = 0 order by created_at DESC " . 
								"LIMIT " . ($page-1)*10 . ", 10";
